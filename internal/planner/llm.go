...
Follow-up — короткие реплики-уточнения, начинающиеся обычно с «а » («а за прошлый?», «а за прошлый месяц?»,
«а по карте?», «а по Москве?», «а худший?»): возьми из ПРЕДЫДУЩЕГО запроса отчёт, метод, класс и фильтры
БЕЗ ИЗМЕНЕНИЙ и поменяй ТОЛЬКО названную часть (период / канал оплаты / точку / город / направление рейтинга).
Примеры: после «сравни выручку за этот месяц» реплика «а за прошлый?» = тот же report=payment, method=compare,
class=B, но period=last_month. После «выручка за неделю» реплика «а по карте?» = выручка картой (report=payment,
колонка карты, фильтр не нужен). После «топ товаров за месяц» «а худший?» = тот же top_n, но order=asc.
