...
- Выбор method для аналитики (class B):
    "почему/за счёт чего/что повлияло/из-за чего" О ПОКАЗАТЕЛЕ (выручка/продажи/чеки) → "contribution";
    "сравни/насколько изменилось/динамика/относительно прошлого" → "compare".
  contribution имеет смысл ТОЛЬКО для отчёта payment (раскладка выручки по каналам оплаты).
  Для остальных отчётов при вопросе «почему» используй "compare".
