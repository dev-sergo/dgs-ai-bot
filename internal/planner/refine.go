...
var adviceRe = regexp.MustCompile(
	`на\s+ч[еёе]м\b[^.?!]*теря|теря[юеё][а-яё]*\s+деньг|где\s+(я\s+)?теря` +
		...
		`|на\s+ч[еёе]м\s+(можно\s+)?с?эконом|дай\s+совет|посоветуй|что\s+посоветуешь`)
...
// Период берём как есть (модель обычно проставляет); пустой → advice-ветка спросит период.
