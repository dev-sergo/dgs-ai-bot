...
.PHONY: tidy get-tg build build-host run-host bot-host test test-live vet run eval-host eval-smoke fmt sh
...
eval-smoke:  ## быстрый smoke-прогон планировщика (~76 кейсов, ~5 мин) — для итерации между полными прогонами
	$(MAKE) eval-host EVAL_PROMPTS=test/eval/smoke.jsonl
